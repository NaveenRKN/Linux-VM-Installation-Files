...
namespace RiskDashboard.Api.Services.Services
{
    public class TokenService : ITokenService
    {
        ...
        public async Task<AuthenticatedResponse> VerifyRefreshTokenAndReGenerate(string refreshToken)
        {
            ...
            List<UserModel> getExistingRefreshToken = await _repoWrapper.UserRepository.FindByCondition(x => x.AuthToken == refreshToken);
            if (getExistingRefreshToken is null)
            {
                return null;
            }
            else
            {
                var newAccessToken = await GenerateNewAccessToken(getExistingRefreshToken[0]);
                getExistingRefreshToken[0].TokenExpiryTime = DateTime.Now.AddDays(7);
                await _userService.UpdateUserToken(getExistingRefreshToken[0]);
                authenticatedResponse = new AuthenticatedResponse
                {
                    AceessToken = newAccessToken,
                    RefreshToken = getExistingRefreshToken[0].AuthToken
                };
            }
